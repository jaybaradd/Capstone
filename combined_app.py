...
def run_earnings_call_analysis(company_name, status_callback=None):
    """
    ...
    try:
        if status_callback:
            status_callback("📥 Fetching audio from YouTube...")
        
        audio_path, video_title, upload_date = download_youtube_audio_and_metadata(company_name)
        
        if status_callback:
            status_callback(f"✅ Found: {video_title}")
            status_callback("🎙️ Transcribing audio...")
        ...
        if status_callback:
            status_callback(f"✅ Transcription complete ({len(raw_transcript)} chars)")
...
def run_financial_reports_analysis(company_name, default_year, report_files, audit_files, 
                                   filename_mapping, status_callback=None):
    """
    ...
    try:
        if status_callback:
            status_callback("📁 Saving uploaded files...")
        ...
        if status_callback:
            status_callback("🔧 Initializing extractor...")
        ...
        if status_callback:
            status_callback("📊 Extracting financial data...")
        
        extractor._extract_financial_data()
        
        if status_callback:
            status_callback("🔍 Extracting audit info...")
        
        audit_summary = extractor._extract_audit_info()
        
        if status_callback:
            status_callback("📋 Creating comparison tables...")
        
        comparison_tables = extractor._create_comparison_tables()
        
        if status_callback:
            status_callback("💡 Generating insights...")
        
        insights = extractor._generate_insights()
        
        if status_callback:
            status_callback("📄 Creating final report...")
        ...
        if status_callback:
            status_callback("✅ Financial Reports Analysis Complete!")
...
def main():
    ...
    if st.button("🚀 Start Comprehensive Analysis", disabled=not can_process, type="primary", key="analyze_all"):
        ...
        # Create status displays for each analysis
        ec_status_placeholder = None
        fr_status_placeholder = None
        ...
        if can_process_ec:
            with col_ec if col_ec else st.container():
                ...
                ec_status_placeholder = st.empty()
        
        if can_process_fr:
            with col_fr if col_fr else st.container():
                st.subheader("📊 Financial Reports Analysis")
                fr_progress_bar = st.progress(0)
                fr_status_placeholder = st.empty()
        
        # Thread-safe status update functions with locks
        ec_lock = threading.Lock()
        fr_lock = threading.Lock()
        
        ec_progress = {'value': 0, 'total_steps': 10}
        fr_progress = {'value': 0, 'total_steps': 8}
        
        def update_ec_status(message):
            """Thread-safe status update for earnings call"""
            if ec_status_placeholder and ec_progress_bar:
                with ec_lock:
                    ec_status_placeholder.info(f"🔄 {message}")
                    ec_progress['value'] = min(ec_progress['value'] + 1, ec_progress['total_steps'])
                    progress_pct = ec_progress['value'] / ec_progress['total_steps']
                    ec_progress_bar.progress(progress_pct)
        
        def update_fr_status(message):
            """Thread-safe status update for financial reports"""
            if fr_status_placeholder and fr_progress_bar:
                with fr_lock:
                    fr_status_placeholder.info(f"🔄 {message}")
                    fr_progress['value'] = min(fr_progress['value'] + 1, fr_progress['total_steps'])
                    progress_pct = fr_progress['value'] / fr_progress['total_steps']
                    fr_progress_bar.progress(progress_pct)
        
        # Execute analyses in parallel using ThreadPoolExecutor
        st.info("⚡ Starting parallel execution...")
        
        # Get the current Streamlit context to pass to threads
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            if can_process_ec:
                ...
                if ctx:
                    add_script_run_ctx(ec_future, ctx)
                futures['earnings_call'] = ec_future
                st.info(f"✅ Earnings Call analysis thread started")
            ...
            if can_process_fr:
                ...
                # Add Streamlit context to the thread
                if ctx:
                    add_script_run_ctx(fr_future, ctx)
                futures['financial_reports'] = fr_future
                st.info(f"✅ Financial Reports analysis thread started")
            
            st.info(f"� Running {len(futures)} analysis threads in parallel...")
            
            # Wait for all futures to complete and collect results
            for name, future in futures.items():
                try:
                    result = future.result()  # This blocks until the thread completes
                    ...
                    if name == 'earnings_call':
                        if result.get('success'):
                            ...
                            if ec_status_placeholder:
                                ec_status_placeholder.success(f"✅ Earnings Call Analysis Complete!")
                        else:
                            st.session_state.ec_results = None
                            if ec_status_placeholder:
                                ec_status_placeholder.error(f"❌ Error: {result.get('error', 'Unknown error')}")
                        ...
                        if result.get('success'):
                            ...
                            if fr_status_placeholder:
                                fr_status_placeholder.success(f"✅ Financial Reports Analysis Complete!")
                        else:
                            st.session_state.fr_results = None
                            if fr_status_placeholder:
                                fr_status_placeholder.error(f"❌ Error: {result.get('error', 'Unknown error')}")
